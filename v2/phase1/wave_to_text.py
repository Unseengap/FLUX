"""
WaveToText: Last-Mile Wave→Bytes Decoder
...
class WaveToText(nn.Module):
    ...
    def forward(
        ...
        Returns:
            ...
            Targets: byte_0, byte_1, ..., byte_{n-1}, EOS
        """
    ...
    def forward_batch(
        ...
        embedded = self.byte_embed(padded_inputs)           # [B, seq_len, 64]
    ...
    def decode_sequence(
        ...
        """
        Decode a sequence of chunk waves to a list of bytes objects.

        Args:
            chunk_waves: [N, 432] batch of chunk waves
            temperature: Sampling temperature
        Returns:
            List[bytes] — one bytes object per chunk
        """
        return [self.decode(w, temperature) for w in chunk_waves]
    ...
    def decode_to_text(
        ...
        """
        Decode chunk waves to a single UTF-8 string.

        Args:
            chunk_waves: [N, 432]
            temperature: Sampling temperature
        Returns:
            Decoded text string (invalid UTF-8 replaced with '?')
        """
        chunks = self.decode_sequence(chunk_waves, temperature)
        raw = b''.join(chunks)
