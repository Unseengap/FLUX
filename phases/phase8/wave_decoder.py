...
class WaveDecoder(nn.Module):
    ...
    def generate(
        ...
        """
        Full autoregressive generation with cross-attention and repetition handling.
        ...
        Args:
            ...
            repetition_penalty: Penalty for repeating recent tokens (1.0 = no penalty)
            no_repeat_ngram_size: Block repeating n-grams of this size (0 = disabled)
        ...
        for _ in range(max_length):
            ...
            if repetition_penalty != 1.0 and len(generated) > 0:
                # Penalize tokens seen in last 32 positions
                recent = generated[-32:]
                for token in set(recent):
                    # Stronger penalty for more recent occurrences
            ...
            if no_repeat_ngram_size > 0 and len(generated) >= no_repeat_ngram_size:
                # Check if any token would create a repeated n-gram
                ...
                for i in range(len(generated) - no_repeat_ngram_size + 1):
                    if tuple(generated[i:i + no_repeat_ngram_size - 1]) == ngram_prefix:
                        # This n-gram prefix appeared before, block the completing token
